...
describe("deleteBlob", async () => {
  ...
  it("should send the correct request", async () => {
    ...
    await deleteBlob(mockServer, mockSha256);
  ...
  it("should include Authorization header if auth is provided", async () => {
    ...
    await deleteBlob(mockServer, mockSha256, { auth: mockAuth });
  ...
  it("should retry with auth when receiving 401 status", async () => {
    ...
    await deleteBlob(mockServer, mockSha256, { onAuth });
  ...
  it("should throw error if 401 received and no onAuth handler provided", async () => {
    ...
    await expect(deleteBlob(mockServer, mockSha256)).rejects.toThrow("Missing auth handler");
  });

  it("should retry with payment when receiving 402 status", async () => {
    const paymentRequest = new PaymentRequest([], "delete-6846354183", 100, "sat", ["https://mint.example.com"]);
    ...
    await deleteBlob(mockServer, mockSha256, { onPayment });
  ...
  it("should retry with generic payment headers", async () => {
    ...
    await deleteBlob(mockServer, mockSha256, { onPaymentRequired });

    expect(onPaymentRequired).toHaveBeenCalledWith(mockServer, mockSha256, expect.any(Headers));
  ...
  it("should throw error if 402 received and no onPayment handler provided", async () => {
    ...
    await expect(deleteBlob(mockServer, mockSha256)).rejects.toThrow("Missing payment handler");
  });
  ...
  it("should respect the AbortSignal", async () => {
    ...
    const promise = deleteBlob(mockServer, mockSha256, { signal });
    controller.abort();
